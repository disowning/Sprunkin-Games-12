...
import { Calendar, Heart, Eye, Play } from 'lucide-react';
...
// 修改这里: 使用 Params 类型，并明确返回类型为 Promise<JSX.Element>
export default async function GamePage(props: Params): Promise<JSX.Element> {
  const { params } = props;
  ...
  // 生成完整的游戏URL（用于分享）
  const gameUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://sprunkin.com'}/game/${game.slug}`;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6">
          ...
          <div className="md:col-span-1">
            ...
            <div className="mt-6 bg-gray-50 p-4 rounded-lg">
              <SocialShare
                title={`Play ${game.title} on Sprunkin`}
                url={gameUrl}
                description={game.description || `Play ${game.title} for free on Sprunkin`}
